...
隐含规则是指Makefile中早先约定不需要再写下来的规则。

隐含规则在目标依赖不明确或者是生成指令不明确时会产生调用，若是目标生成的规则很清晰，那么就不会调用隐含规则。

其次，隐含规则的调用由顺序而言（隐含规则库的有效性有优先级），例如：在一个目录下有makefile，foo.c，foo.p三个文件，makefile里内容为：

```makefile
foo.o:foo.p
```

由于foo.o生成命令并不明确，因而会触发隐含规则。但在隐含规则中C编译器生成目标文件的优先级高于Pascal编译器，故最终生成目标文件命令为：

```makefile
cc -c $(CFLAGS) foo.c
```

取消预设值的隐含规则：make -r/--no-builtin-rules

取消定义变量对隐含规则的作用：make -R / --no builtin-variables

***隐含规则在原文中9.2 P60处。***

隐含规则的变量：命令相关如$(CC)；参数相关如$(CFLAGS)
...
上述语句并没有指明相应的执行规则，因而会触发隐含规则，我们想象中的规则是：

```makefile
$(CC) -c $(CFLAGS) x.c
$(CC) -c $(CFLAGS) y.c
#以及
$(CC) -o target x.o y.o $(CFLAGS) $(LDFLAGS)
```

但实际上若实际运行makefile，文件夹下没有x.c以及y.c，而是有x.y以及y.y文件，那么实际上生成流程是:.y---->.c----->.o----->target，这就叫触发了隐含规则链。相应的.c文件被称作是中间目标（.o算中间目标么？感觉算是普通目标而非中间目标）

与一般的目标不同：1.中间目标不存在才会触发中间规则2.最终目标生成成功后，产生的中间目标文件会用rm -f命令删除

可以用.INTERMEDIATE伪目标标签显式声明中间目标

可以用.SECONDARY阻止make自动删除中间目标

可以以模式的方式将文件指明为.PRECIOUS的依赖目标，保存中间目标文件

隐含规则链中，一个目标出现两次及以上（防止递归）

某些目标生成在make中可以被优化，例如C源文件生成可执行文件，但实际上可以由编译器一条命令生成完毕。

#### 模式规则

%的展开发生在makefile运行时，函数或变量的展开发生在makefile载入到make中时。（说明%的展开晚于变量以及函数的展开）
